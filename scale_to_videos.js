...
if (getArgumentValue === undefined) {
  photo_path_to_frames =
    "./Python-Scripts/video/bluetooth/";
} else {
...
async function cascading_images_pose_estimation(photo_path, choice) {
  console.log(
    "Starting to estimate pose values for list of Video frames in the given path : " +
      photo_path
  );
  let config;
  if (choice === 1) {
  ...
  fs.readdir(photo_path, async function (err, items) {
    if (err) {
      console.error("error reading photo path: ", err);
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (path.extname(items[i]) == ".png") {
          console.log(items[i]);
          func_path = photo_path;

          // let length = fs.readdirSync(func_path).length;
          // for (let i = 0; i < length - 1; i++) {
          const imgPath = func_path + "/" + i + ".png"
          ...
          let pose = await single_net.estimateSinglePose(
            input,
            imageScaleFactor,
            flipHorizontal,
            outputStride
          ...
          console.log("writing key_points.json at: ", func_path + "key_points.json")

          // TODO: this is overwriting the key_points.json file each iteration. Probably need a new file for each image.
          fs.appendFileSync(
            func_path + "key_points.json",
            JSON.stringify(pose_list)
          );
          console.log(
            'Key Points File for "' + items[i] + '" has been created'
          );
        }
      }
    } catch (err) {
...
readline.question(
  ...
  (name) => {
    cascading_images_pose_estimation(photo_path_to_frames, name).then(
      (r) => console.log("Key Points generated Successfully."),
      () => console.log("Error occurred in generating keypoints.")
    );
    readline.close();
